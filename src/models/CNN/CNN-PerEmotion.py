...
# Paths
split_dir = os.path.join(os.path.dirname(__file__), "../luca-split/split-csv/")
emote_dir = os.path.join(os.path.dirname(__file__), "../luca-split/per-emotion/")
...
# Load CSV's
train_df = pd.read_csv(os.path.join(split_dir, f"train.csv"))
test_df = pd.read_csv(os.path.join(split_dir, f"test.csv"))
...
for emote in emotions:
    ...
    # Make emotion-specific dataloader
    emote_df = pd.read_csv(os.path.join(emote_dir, f"{emote}.csv"))
    emote_dataset = Extract(emote_df, img_dir)
    emote_loader = DataLoader(emote_dataset)
    
    print(f"{emote}: {model.score(emote_loader)}")
